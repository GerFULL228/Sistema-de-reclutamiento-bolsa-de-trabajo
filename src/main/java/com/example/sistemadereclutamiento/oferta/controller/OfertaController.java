...
public class OfertaController {
    ...
    public ResponseEntity<Page<OfertaResponseDTO>> listarOfertas(
        ...
        return ResponseEntity.ok(ofertaService.obtenerTodas(pageable));
    }
    ...
    public ResponseEntity<OfertaResponseDTO> crearOferta(@RequestBody OfertaRequestDTO requestDTO) {
        OfertaResponseDTO nuevaOferta = ofertaService.guardarOferta(requestDTO);
        return new ResponseEntity<>(nuevaOferta, HttpStatus.CREATED);
    }
 
    @PatchMapping("/{id}")
    @PreAuthorize("hasAuthority('OFERTA_UPDATE')")
    public ResponseEntity<OfertaResponseDTO> actualizarOferta(
            @PathVariable Long id,
            @RequestBody OfertaUpdateDTO requestDTO) {
 
        return ResponseEntity.ok(ofertaService.actualizarOferta(id, requestDTO));
    }
 
    @DeleteMapping("/{id}")
    @PreAuthorize("hasAuthority('OFERTA_DELETE')")
